...
final class Helpers
{
	 ...
	 * @var string
	 */
	static $text_domain = WWAPW_DOMAIN;
	...
	/**
	 * Check if yith multi-vendor plugin is activated.
	 *
	 * @return bool
	 */
	public static function is_yith_vendor_active()
	{
		// including path to use is_plugin_Active
		include_once( ABSPATH . 'wp-admin/includes/plugin.php' );
		...
		if ( function_exists( 'is_plugin_active' ) )
		{
			...
			return is_plugin_active( 'yith-woocommerce-multi-vendor-premium/init.php' );
		}

		// check if yith plugin main class exists.
		return class_exists( 'YITH_Vendors' );

	}

	/**
	 * Getting vendor role.
	 * According to the current active plugin.
	 *
	 * @return string
	 */
	public static function get_vendor_role()
	{
		// check if yith multi-vendor plugin is activated.
		if ( self::is_yith_vendor_active() )
		{
			// yith vendor role.
			return 'yith_vendor';
		}

		// default, woocommerce vendor role.
		return 'wc_product_vendors_admin_vendor';
	}
	...
	/**
	 * Get Excluded category from wizard dropdown.
	 *
	 * @return int
	 */
	public static function get_excluded_categories()
	{
		// categories to be excluded from wizard.
		$cats = false;

		/**
		 * Categories to be excluded from categories
		 * drop-down menu in add product wizard.
		 */
		return apply_filters( 'wwapw_excluded_categories', $cats );
	}
}